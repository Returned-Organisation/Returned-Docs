...
1. Click **Build Test** to create ``Builds/Test/Unturned.exe``. Steam sync, content links, and the prefab bundle run automatically.
2. Run ``Builds/Test/Unturned.exe``.

If assets are missing in ``Builds/Shared/``, click **Link Retail Game Content (Steam)** first.

.. tip::

	If the game crashes on startup with a corrupted ``level0`` error, click **Restore SDK Unturned_Data (Fix Merge)** and then **Build Test (Scripts Only)**.
